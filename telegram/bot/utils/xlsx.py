...
def _cell_xml(row: int, column: int, value: object) -> str:
    ...
    if value is None or value == "":
        return (
            f"<c r=\"{address}\" t=\"inlineStr\"><is><t xml:space=\"preserve\"></t></is></c>"
        )
    if isinstance(value, (int, float)) and not isinstance(value, bool):
        return f"<c r=\"{address}\" t=\"n\"><v>{value}</v></c>"
    text = escape(str(value))
    return (
        f"<c r=\"{address}\" t=\"inlineStr\"><is><t xml:space=\"preserve\">{text}</t></is></c>"
    )
...
def build_simple_table_xlsx(headers: Sequence[object], rows: Iterable[Sequence[object]]) -> bytes:
    ...
    with ZipFile(buffer, "w", ZIP_DEFLATED) as archive:
        ...
        archive.writestr("docProps/app.xml", _DOC_PROPS_APP_XML)
        archive.writestr("docProps/core.xml", _DOC_PROPS_CORE_XML)
